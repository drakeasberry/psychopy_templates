...
expName = u'discrimination_axb'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/casillas/code/psychopy/psychopy_templates/discrimination_axb/discriminacion_axb.psyexp',
...
for thisTrial in trials:
    ...
    sound_stim3.setSound(stim3, secs=0.25)
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if t >= 1.75 and sound_stim3.status == NOT_STARTED:
            # keep track of start time/frame for later
            sound_stim3.tStart = t
            sound_stim3.frameNStart = frameN  # exact frame index
            sound_stim3.play()  # start the sound (it finishes automatically)
        frameRemains = 1.75 + 0.25- win.monitorFramePeriod * 0.75  # most of one frame period left
        if sound_stim3.status == STARTED and t >= frameRemains:
            sound_stim3.stop()  # stop the sound (if longer than duration)
        
        # *text_response_first* updates
        if t >= 2.0 and text_response_first.status == NOT_STARTED:
            # keep track of start time/frame for later
            text_response_first.tStart = t
            text_response_first.frameNStart = frameN  # exact frame index
            text_response_first.setAutoDraw(True)
        frameRemains = 2.0 + 2.75- win.monitorFramePeriod * 0.75  # most of one frame period left
        if text_response_first.status == STARTED and t >= frameRemains:
            text_response_first.setAutoDraw(False)
        
        # *text_response_third* updates
        if t >= 2.0 and text_response_third.status == NOT_STARTED:
            # keep track of start time/frame for later
            text_response_third.tStart = t
            text_response_third.frameNStart = frameN  # exact frame index
            text_response_third.setAutoDraw(True)
        frameRemains = 2.0 + 2.75- win.monitorFramePeriod * 0.75  # most of one frame period left
        if text_response_third.status == STARTED and t >= frameRemains:
            text_response_third.setAutoDraw(False)
        
        # *text_response_same* updates
        if t >= 2.0 and text_response_same.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        if t >= 2.0 and key_resp_trial.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        if key_resp_trial.status == STARTED:
            theseKeys = event.getKeys(keyList=['1', '0', 'space'])
